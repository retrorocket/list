...
helper mango  => sub { state $mango = Mango->new("mongodb://localhost:".$config->{mongo_port}) };
helper pastes => sub { shift->mango->db($config->{data_base})->collection($config->{collection}) };
...
get '/check' => sub {
 ...
 #$self->app->log->error(Dumper $self->pastes->find({"screen_name" => $screen_name}));
    ...
    $self->session( check_code => $check_code ); #チェックポイントフラグ
    
    # 前回無事に終了している
    if($check_code == 200) {
            $self->redirect_to( $self->url_for('index')->to_abs->scheme('https') );
    }
    elsif ($check_code == 429) {
        # 反省部屋送り
        return $self->render(
            check_code => 429,
            message => "前回API切れを起こしています。前回リストを作成してから15分以上経過していない場合、作成ページへ進んでもリストの作成に失敗することがあります。"
		);
    }
        ...
        return $self->render(
            check_code => 404,
            message => "前回原因が不明な404エラーが発生しています。作成ページへ進んでもリストの作成に失敗することがあります。"
		);
    }
    elsif($check_code == 1) {
        # 実況部屋送り
        return $self->render(
            check_code => 1,
            message => "このアカウントで現在リストを作成中です。作成ページヘ進むと現在の状況を確認することができます。"
		);
    }
    else {
        $self->redirect_to( $self->url_for('index')->to_abs->scheme('https') );
...
get '/progress' => sub {
	...
	return $self->render(json =>{'result' => "You did not create the list in this account.", 'complete' => -1}) unless ($screen_name);
    ...
    # 名前がない
    unless($doc_name){
        return $self->render(json =>{'result' => "You did not create the list in this account."
        , 'complete' => -1});
    }
    
    # 状況に応じてjsonを返却する
    return $self->render(json =>{'complete' => $doc->{complete}, 'result' => $doc->{result} });  
} => 'progress';
...
post '/list' => sub {
	...
	unless ($access_token && $access_token_secret && $check_code) {
		return $self->render(json =>{'result' => "You are Not authorized.", 'complete' => -1});
	}
